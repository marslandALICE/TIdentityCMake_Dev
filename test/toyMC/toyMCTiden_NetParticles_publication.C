...
void PlotdEdx()
{
  ...
  for (Int_t ipart = 0; ipart<nParticles; ipart++) {
    ...
    fShape[ipart]->SetLineWidth(2);
    fShape[ipart]->SetNpx(10000);
  }
  //
  //
  ...
  leg->AddEntry(fShape[0], "e^{-}, e^{+}"    ,"L");
  leg->AddEntry(fShape[1], "#pi^{-}, #pi^{+}"  ,"L");
  leg->AddEntry(fShape[2], "K^{-}, K^{+}"    ,"L");
  leg->AddEntry(fShape[3], "#bar{p}, p"    ,"L");
  leg->AddEntry(fShape[4], "#bar{d}, d"    ,"L");
  //
  //
  ...
  h0->Draw("E");
  for (Int_t ipart = 0; ipart<nParticles; ipart++) fShape[ipart]->Draw("same");
  leg->Draw("same");
  line->Draw();
